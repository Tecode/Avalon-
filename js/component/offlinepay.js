 ...
 */
define(['bootstrap', 'avalon', 'jstree','cropbox', 'jquery_select', 'sweet_alert', 'featurepack'], function (bootstrap, avalon, jstree,cropbox, jquery_select, swal, featurepack) {
    ...
    var cloudMail = {
            ...
            offlinepay = avalon.define({
                ...
                validate: featurepack.pack.checkValue(function () {
                    ...
                    if ($('.select-user').select('getSelected').length == 0) {
                        ...
                        postdata.logoUrl = offlinepay.logoUrl;
        ...
        sendFile: function () {
            ...
            $.ajax({
                data: data,
                type: "POST",
                url: dataUrl.logoUrl,//上传地址
                cache: false,
                contentType: false,
                processData: false,
                success: function (resule) {
                    swal("添加成功!", "您已经添加logo，点击OK关闭窗口。", "success");
                    imgData = null;
                    offlinepay.logoUrl = resule.url;
                }
            });
        },
