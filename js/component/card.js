...
define(['avalon', 'bootstrap', 'bootstrap_select', 'moment', 'daterangepicker', 'featurepack', 'plupload', 'sweet_alert'], function (avalon, bootstrap, selectpicker, moment, daterangepicker, featurepack, plupload, swal) {
    ...
    var cloudMail = {
        ...
        getResponse:function () {
            featurepack.pack.ajax(dataUrl.getResponseUrl, "get", postdata, function (result) {
                if (result.code == 0) {
                    //参数有问题和上面的一样
                    vipCard.vipData = result.data.vipData;
                    vipCard.enterdata = result.data.enterdata;
                } else {
    ...
    var initStart = function (url) {
        try {
            alert(getQueryString(joincode));
            if(getQueryString('joincode') == 'joincode'){
            }
        }catch (e){
